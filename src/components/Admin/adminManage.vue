        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              size="mini"
              type= "info"
              v-if= "+scope.row.isadmin === 0 ? true : false"
              @click="handleReview(scope.$index, scope.row)">{{+scope.row.isadmin === 0 ? '用户' : '管理员'}}</el-button>
            <el-button
              size="mini"
              type="success"
              v-else
...
export default {
  ...
  methods: {
    ...
    handleReview (index, row) {
      ...
      axios.post('/api/userAdmin/isadmin', qs.stringify(params))
        .then((response) => {
          if (response.data.code === 200) {
            this.$message({// notify
              type: 'success',
